...
export const zhCN = {
  ...
  offlineReading: {
    ...
    settings: {
      ...
      disableConfirm: "关闭离线阅读将清除此设备的离线文章和图片。服务器上的文章状态不会改变，是否继续？",
      disablePendingConfirm: (count: number) => `还有 ${count} 个待同步操作。关闭离线阅读会清除缓存并永久丢失这些操作，仍要继续吗？`,
      enabled: "离线阅读已开启，将在后台准备缓存。",
      disabled: "离线阅读已关闭，本机离线缓存已清除。",
      toggleFailed: "离线阅读设置更新失败，请重试。",
...
export const enUS = {
  ...
  offlineReading: {
    ...
    settings: {
      ...
      disableConfirm: "Turning off offline reading clears offline articles and images from this device. Server article states will not change. Continue?",
      disablePendingConfirm: (count: number) => `${count} actions are waiting to sync. Turning off offline reading clears the cache and permanently loses them. Continue?`,
      enabled: "Offline reading is enabled. The cache will be prepared in the background.",
      disabled: "Offline reading is disabled and this device's offline cache was cleared.",
      toggleFailed: "Offline reading could not be updated. Try again.",
...
export const jaJP = {
  ...
  offlineReading: {
    ...
    settings: {
      ...
      disableConfirm: "オフライン閲覧を無効にすると、この端末のオフライン記事と画像が消去されます。サーバー上の記事状態は変更されません。続行しますか？",
      disablePendingConfirm: (count: number) => `${count} 件の操作が同期待ちです。オフライン閲覧を無効にするとキャッシュが消去され、これらの操作は永久に失われます。続行しますか？`,
      enabled: "オフライン閲覧を有効にしました。キャッシュはバックグラウンドで準備されます。",
      disabled: "オフライン閲覧を無効にし、この端末のキャッシュを消去しました。",
      toggleFailed: "オフライン閲覧の設定を更新できませんでした。もう一度お試しください。",
