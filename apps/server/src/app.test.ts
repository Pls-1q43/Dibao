...
describe("server API vertical slice", () => {
  ...
  it("keeps recommended pagination on the cursor rank context", async () => {
    const db = createFixtureDatabase();
    const embeddings = new SqliteEmbeddingRepository(db);
    embeddings.upsertProvider({
      id: "provider_page",
    ...
    try {
      const firstPage = await app.inject({
        method: "GET",
        url: "/api/articles?view=recommended&limit=1"
      });
      expect(firstPage.statusCode, firstPage.body).toBe(200);
      expect(firstPage.json().data.map((article: { id: string }) => article.id)).toEqual([
        "article_recommended"
      ]);
      ...
      const secondPage = await app.inject({
        method: "GET",
        url: `/api/articles?view=recommended&limit=1&cursor=${firstPage.json().page.nextCursor}`
      });
